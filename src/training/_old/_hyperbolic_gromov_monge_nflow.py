...
class HyperbolicGromovMongeEmbeddingInformedNFlowTraining(torch.nn.Module):
    def __init__(self, embedding_model, nflow_diffeomorphism, reg_param=None):
    ...
    def loss(self, x):
        ...
        # sparsity loss in the second half of the dimensions
        if self.reg_param is None:
            return recon_loss
        phi_x = self.phi.forward(x)  # N x d
        l1_loss = torch.mean(torch.abs(phi_x[:, self.emb.d:]).sum(dim=1))
        
        # total loss
        loss = recon_loss + self.reg_param * l1_loss
        return loss