...
class UpperTriangularLinearTransform(Transform):
    def __init__(self, features, bias=False, unit_det=False, efficient_inverse=False):
        super().__init__()
        self.features = features
        self.weight = nn.Parameter(torch.triu(torch.eye(features, features)))
        if bias:
    ...
    def _forward(self, x, context=None): 
        if self.unit_det:
            z = x @ (torch.triu(self.weight.to(x.device), diagonal=1) + torch.eye(self.features).to(x.device)).t()
            log_abs_det = torch.zeros(1, device=x.device)
        else:
            z = x @ (torch.triu(self.weight).to(x.device)).t()
    ...
    def _inverse(self, z, context=None):
        if self.unit_det:
            x = torch.matmul(z - self.bias.to(z.device), torch.inverse(torch.triu(self.weight.to(z.device), diagonal=1) + torch.eye(self.features).to(z.device)).t())
            log_abs_det = torch.zeros(1, device=z.device)
        else:
            x = torch.matmul(z - self.bias.to(z.device), torch.inverse(torch.triu(self.weight).to(z.device)).t())
