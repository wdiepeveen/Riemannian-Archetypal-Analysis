...
class UpperTriangularLinearVectorTransform(LinearVectorTransform):
    def __init__(self, d, bias=False, efficient_inverse=False):
        super().__init__(d)

        # Strictly upper-triangular part is learnable; diagonal is fixed to 1
        init = torch.triu(torch.zeros(self.d, self.d), diagonal=1)
    ...
    def _weight_matrix(self, x):
        # Construct full upper-triangular matrix with ones on the diagonal
        eye = torch.eye(self.d, device=x.device, dtype=x.dtype)
        return torch.triu(self.weight, diagonal=1) + eye

    def _forward(self, x, context=None):
        W = self._weight_matrix(x)
        z = x @ W.t()
        z = z + self.bias  # bias is a Parameter or buffer; already on correct device
    ...
    def _inverse(self, z, context=None):
        ...
        x = z - self.bias  # bias is a Parameter or buffer; already on correct device
