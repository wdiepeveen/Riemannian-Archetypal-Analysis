...
from src.radials.unimodal.inverse_affine_norm.pre_bias_linear import InversePreBiasLinearNormRadial

class MultiInversePreBiasLinearNormRadial(MultiInverseAffineNormRadial):
    def __init__(self, d, num_radials, centered=True):
        super().__init__(d, [InversePreBiasLinearNormRadial(d, centered=centered) for _ in range(num_radials)])
