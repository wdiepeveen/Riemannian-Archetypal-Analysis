...
class MultiOffCenteredTrimmedEllipsoidRadial(MultiModalRadial):
    def __init__(self, covs, mus, c=4/3, p=None, aggregation='max'):
        if p is None:
            radials = [EllipsoidEnclosingOffCenteredTrimmedEllipsoidRadial(cov, mu, c=c) for cov, mu in zip(covs, mus)]
        else:
            radials = [GaussianEnclosingOffCenteredTrimmedEllipsoidRadial(cov, mu, c=c, p=p) for cov, mu in zip(covs, mus)]
