...
class ConvexSimplexStructuredMatrixFactorizationSolver(DimensionReductionSolver):
    """ Base class for convex simplex structured matrix factorization of d-dimensional data X = (X_1, ..., X_n) \in R^{d x n} """
    ...
    def fit(self,
        ...
        if N_max > 0 and N > N_max:
            ...
            self.fit(Y, r, N_max=-1, max_iter=max_iter, alpha_F_init=alpha_F_init, alpha_G_init=alpha_G_init,
                    ...
                    bt_max_steps=bt_max_steps, tol=tol, init=init, save_history=save_history, save_G=False)
            
            # compute G for full data
            device, dtype = X.device, X.dtype

            if init == "random":
                G = torch.rand(r, X.shape[1], device=device, dtype=dtype)
                G = G / G.sum(dim=0, keepdim=True).clamp_min(1e-12)
            elif init == "uniform":
                G = torch.full((r, X.shape[1]), 1.0 / r, device=device, dtype=dtype)
            else:
            ...
            if save_history:
                history_given_V.append(fval_prev)
            ...
            if save_history:
                self.history_2 = torch.tensor(history_given_V, device=device, dtype=dtype)

            print(f"CSSMF solver finished phase II after {k+1} iterations with objective value {fval:.4f} and relative change {rel:.2f}")
        else:
            device, dtype = X.device, X.dtype

            if init == "random":
                F = torch.rand(N, r, device=device, dtype=dtype)
                F = F / F.sum(dim=0, keepdim=True).clamp_min(1e-12)
                G = torch.rand(r, N, device=device, dtype=dtype)
                G = G / G.sum(dim=0, keepdim=True).clamp_min(1e-12)
            elif init == "uniform":
                F = torch.full((N, r), 1.0 / N, device=device, dtype=dtype)
                G = torch.full((r, N), 1.0 / N, device=device, dtype=dtype)
            else:
                raise ValueError("init must be 'random' or 'uniform'")
            ...
            if save_G:
                self.labels = torch.argmax(G, dim=0)
            if save_history:
                self.history = torch.tensor(history, device=device, dtype=dtype)

            if save_G:
                print(f"CSSMF solver finished after {k+1} iterations with objective value {fval:.4f} and relative change {rel:.2f}")
            else:
                print(f"CSSMF solver finished phase I after {k+1} iterations with objective value {fval:.4f} and relative change {rel:.2f}")
