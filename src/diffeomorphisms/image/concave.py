...
class ConcaveImageDiffeomorphism(ImageDiffeomorphism):
    def __init__(self, in_channels, height, width, concave):
    ...
    def forward(self, x):
        ...
        """
        eps = 1e-12
        r = torch.linalg.norm(x.reshape(-1, self.d), dim=-1)              

        # safe theta
        theta = torch.where(
            (r > eps)[..., None, None, None],
            x / (r + eps)[..., None, None, None],
            torch.zeros_like(x),
        )                                         

        v_r = self.v(r)                          
        return theta * v_r[..., None, None, None]  
    ...
    def inverse(self, y):
        ...
        """
        eps = 1e-12
        r = torch.linalg.norm(y.reshape(-1, self.d), dim=-1)         

        # safe theta
        theta = torch.where(
            (r > eps)[..., None, None, None],
            y / (r + eps)[..., None, None, None],
            torch.zeros_like(y),
        )                                            
